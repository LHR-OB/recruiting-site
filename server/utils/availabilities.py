...
def get_availability(db: Session, availability_id: str) -> models.Availability:
    return db.query(models.Availability).filter(models.Availability.id == availability_id).first()


def get_availabilities_by_user(db: Session, user_id: str) -> List[models.Availability]:
    return db.query(User).filter(User.id == user_id).first().availabilities


def get_availabilities_by_system(db: Session, system_id: str) -> List[models.Availability]:
...
def update_availability(db: Session, availability_id: str, availability: schemas.AvailabilityUpdate) -> models.Availability:
...
def delete_availability(db: Session, availability_id: str) -> bool:
    db_availability = db.query(models.Availability).filter(
        models.Availability.id == availability_id).first()
    if db_availability is None:
        return False
